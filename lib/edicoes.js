...
// Auditoria: quem editou (Master aparece destacado, inclusive em itens já decididos) e o que mudou
function acaoEdicao(usuario, comoAprovador, item, tipo, statusPendente) {
  if (usuario?.perfil === "master" && item?.status && item.status !== statusPendente) return `Editou ${tipo} já decidido(a) (Master)`;
  if (usuario?.perfil === "master") return `Editou ${tipo} (Master)`;
  return comoAprovador ? `Editou ${tipo} (aprovação)` : `Editou ${tipo} próprio(a)`;
}
const detalheMudancas = (mudancas) => (mudancas.length ? ` · ${mudancas.join("; ")}` : " · sem alterações");
...
export async function salvarEdicaoRdo({ rdo, novo, ocorrencias, pis, usuario, comoAprovador, avisar }) {
  ...
  await registrarLog(usuario, acaoEdicao(usuario, comoAprovador, rdo, "RDO", "pendente"), `${pi?.codigo} — RDO de ${fmtData(rdo.data)}${detalheMudancas(mudancas)}`);
...
export async function salvarEdicaoHoras({ registro, novo, pis, usuario, comoAprovador }) {
  ...
  await registrarLog(usuario, acaoEdicao(usuario, comoAprovador, registro, "lançamento de horas", "pendente"), `${codigo(registro.pi_id)} — ${fmtData(registro.data)}${detalheMudancas(mudancas)}`);
...
export async function salvarEdicaoSolicitacao({ s, novo, etapas, usuario, comoAprovador }) {
  ...
  const etapa = etapas.find((e) => e.id === s.etapa_id);
  await registrarLog(usuario, acaoEdicao(usuario, comoAprovador, s, "solicitação de alteração", s.status?.startsWith("pendente") ? s.status : "pendente_coordenador"), `${etapa?.nome || "Etapa"}${detalheMudancas(mudancas)}`);
...
export async function salvarEdicaoOcorrencia({ oc, novo, pis, usuario, comoAprovador, avisar }) {
  ...
  await registrarLog(usuario, acaoEdicao(usuario, comoAprovador, oc, "ocorrência", "pendente"), `${pi?.codigo} — ${oc.categoria}${detalheMudancas(mudancas)}`);
