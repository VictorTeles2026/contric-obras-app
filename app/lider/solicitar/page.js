  ...
  return (
    <MobileShell nav={NAV}>
      <div className="p-5 flex flex-col gap-5">
        <div className="font-head font-bold text-xl">Solicitar alteração</div>

        {ok && <div className="text-sm text-green bg-green/10 rounded-lg px-4 py-3 font-semibold leading-relaxed">✓ Solicitação enviada para o Coordenador.</div>}

        <div className="bg-white rounded-xl border border-line p-4 flex flex-col gap-4">
          <div>
            <div className="text-sm text-muteddim mb-1.5">Etapa</div>
            <select value={etapaId} onChange={(e) => setEtapaId(e.target.value)} className="w-full px-3 py-2.5 rounded-lg border border-line text-base">
              <option value="">Selecione...</option>
              {minhasEtapas.map((e) => <option key={e.id} value={e.id}>{e.nome}</option>)}
            </select>
          </div>
          <div>
            <div className="text-sm text-muteddim mb-1.5">O que quer mudar</div>
            <select value={campo} onChange={(e) => setCampo(e.target.value)} className="w-full px-3 py-2.5 rounded-lg border border-line text-base">
              {CAMPOS.map(([v, l]) => <option key={v} value={v}>{l}</option>)}
            </select>
          </div>
          <div>
            <div className="text-sm text-muteddim mb-1.5">Novo valor proposto</div>
            <input value={valorProposto} onChange={(e) => setValorProposto(e.target.value)} className="w-full px-3 py-2.5 rounded-lg border border-line text-base" />
          </div>
          <div>
            <div className="text-sm text-muteddim mb-1.5">Justificativa</div>
            <textarea value={justificativa} onChange={(e) => setJustificativa(e.target.value)} rows={3} className="w-full px-3 py-2.5 rounded-lg border border-line text-base" />
          </div>
          <button onClick={enviar} disabled={!podeEnviar || enviando} className="py-3.5 rounded-lg bg-cyan text-white text-base font-semibold disabled:opacity-50">
            {enviando ? "Enviando..." : "Enviar solicitação"}
          </button>
        </div>

        <div className="text-sm font-mono text-muteddim tracking-wide mt-1">MINHAS SOLICITAÇÕES</div>
        <div className="flex flex-col gap-2.5">
          {minhasSolicitacoes.map((s) => (
            <div key={s.id} className="bg-white rounded-xl border border-line p-4 text-sm">
              <div className="font-semibold text-base">{s.campo_alterado}: {s.valor_atual || "—"} → {s.valor_proposto}</div>
